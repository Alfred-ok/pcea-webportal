...
const HolyCommunionReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_THIRD_BASE_URL}/reports/all?type=Communion`, {
          headers: {
        ...
        setCommunionData(result.serviceData || []);
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [selectedDistrict, selectedGender, selectedAgeGroup,service, fromDate,selectedDate, toDate, communionData]);
  ...
  const handleDownload = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "HolyCommunionReport");
    XLSX.writeFile(wb, "HolyCommunionReport.xlsx");
  };
...
export default HolyCommunionReport;
