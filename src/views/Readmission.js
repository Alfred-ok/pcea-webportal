...
const Readmission = () => {
  ...
  const tableHeaders = {
    movedToPastoral: ["Full Name", "District", "Gender", "ZpNumber", "Telephone", approval],
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      status: Number(registrationStatus),
      comments: comment,
    };
          ...
          <CTableBody>
            {members[activeTab].map((person, index) => (
              <CTableRow key={person.id}>
                ...
                <CTableDataCell>{person.fullName}</CTableDataCell>
                <CTableDataCell>{person.district}</CTableDataCell>
                <CTableDataCell>{person.gender}</CTableDataCell>
                <CTableDataCell>{person.zPNumber}</CTableDataCell>
                <CTableDataCell>{person.mobileNumber}</CTableDataCell>
...
>
  ...
  <option value="8">Moved To Pastoral</option>
  {
...
>
  ...
  <option value="10">Move to Full Member awaiting Admission</option>
  <option value="9">Moved To Catechism</option>
