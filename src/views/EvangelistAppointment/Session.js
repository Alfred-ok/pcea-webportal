    ...
    const fetchAppointments = async () => {
      try {
        const response = await fetch(
          //"http://197.232.170.121:8594/api/registrations/GetAppointmentsbydepartment?department=Session"
          `${import.meta.env.VITE_BASE_URL}/GetAppointmentsbydepartment?department=Session`
        );
        if (!response.ok) {
